...
description: "Carousel Interaction from [Collins](https://wearecollins.com/case-studies/bose), heavily inspired by [Dimi's](https://x.com/dimitribouquin/status/1994085212469248020?s=20) implementation."
---
...
<Tabs items={['Preview', 'Code']}>
  <Tab value="Preview" className="w-full bg-dot-pattern rounded-xl border mt-4 overflow-hidden p-0">
    <ComponentPreview className="min-h-[560px]">
      <CollinsCarousel />
    </ComponentPreview>
  </Tab>
  <Tab value="Code">
...
function CarouselItem({
  ...
  return (
    <motion.li {...props} style={{ ...props.style, zIndex }} className="w-full h-full">
      <motion.div style={{ x: edgeX }} className="size-full">
...
```tsx
import { CollinsCarousel } from "@/components/ui/collins-carousel";

export default function App() {
  return <CollinsCarousel />;
}
```
