...
import { CarouselProps, TickerContextValue, TickerItemContextValue, CarouselContextValue } from "./types";
...
const InternalCarouselContext = createContext<InternalCarouselContext | null>(null);
...
function CarouselItemWrapper({
    ...
    const { springOffset, containerWidth, itemWidths, gap, layout } = internalContext;
    ...
    const totalItemLength = useMemo(() => {
        return itemWidths.reduce((sum, w, i) => sum + w + (i < itemWidths.length - 1 ? gap : 0), 0);
    }, [itemWidths, gap]);
...
export function Carousel({
    ...
    const itemCenters = useMemo(() => {
        ...
        for (let i = 0; i < itemWidths.length; i++) {
            centers.push(pos + itemWidths[i] / 2);
            pos += itemWidths[i] + gap;
        }
        return centers;
    }, [itemWidths, gap]);
    ...
    const snapToNearest = useCallback(
        (velocity: number = 0) => {
            ...
            const targetItemCenter = itemCenters[nearestIndex];
        ...
        [itemCenters, containerWidth, rawOffset, loop, items.length, minOffset, maxOffset, momentumMultiplier]
    );
    ...
    const handlePan = useCallback(
        (_: PointerEvent, info: PanInfo) => {
            const delta = (axis === "x" ? info.delta.x : info.delta.y) * dragSensitivity;
    ...
    const carouselContextValue: CarouselContextValue = useMemo(
        ...
        [springOffset, totalItemLength, items.length, nextPage, prevPage, gotoPage, currentPage, totalPages, isNextActive, isPrevActive]
    );
